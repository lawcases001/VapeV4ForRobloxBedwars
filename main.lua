...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/lawcases001/VapeV4ForRobloxBedwars/'..readfile('newvape/profiles/commit.txt')..'/'..select(1, path:gsub('newvape/', '')), true)
		end)
		...
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			getgenv().script_key = script_key
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/lawcases001/VapeV4ForRobloxBedwars/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
				end
			]]
			if shared.VapeDeveloper then
...
if not shared.VapeIndependent then
	...
	if isfile('newvape/games/'..game.PlaceId..'.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/lawcases001/VapeV4ForRobloxBedwars/'..readfile('newvape/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true)
			end)
